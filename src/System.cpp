...
int System::FindRuns(QueueArray& queues) {
  ...
  for (StrArray::const_iterator it = runDirs.begin(); it != runDirs.end(); ++it)
  {
    ...
    Runs_.AddRun( Run() );
    if (Runs_.Set_back().SetupExisting( *it, mdInterface_.Package(), submitter_.LocalQueue(), creator_.Topology_AbsolutePath() )) {
...
int System::RefreshCurrentRuns(bool verbose) {
  ...
  for (RunArray::iterator run = Runs_.begin(); run != Runs_.end(); ++run)
  {
    ...
    if (run->Refresh( mdInterface_.Package(), submitter_.LocalQueue(), creator_.Topology_AbsolutePath() )) {
...
int System::RefreshSpecifiedRun(int ridx) {
  ...
  Runs_.Set_Run(ridx).Refresh( mdInterface_.Package(), submitter_.LocalQueue(), creator_.Topology_AbsolutePath() );
  // Change directory back
  if (ChangeToSystemDir()) return 1;
  return 0;
}
